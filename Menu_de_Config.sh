...
do
		...
		echo "		8.Instalacion Github"
		echo "		9.Instalacion Docker"
		echo -e "		10.Salir de este menu\n"
	...
	case "$opcion" in
		...
		"8")
			comment=$1
			rama=$2
			read -p "Desea conectarse con Git (Y/N)?: " conect
			if [[ $conect =~ ^[Yy]$ ]]
				then
					read -p "Digite su correo (user.email): " email
					read -p "Digite el nombre de usuario (user.name): " usuario
			...
			# Ejecucion
				...
				read -p "Desea hacer la clonacion de un repositorio del Git (Y/N)?: " clone
				if [[ $clone =~ ^[Yy]$ ]]
					then
				...
				read -p "Desea conocer el estado del Git (Y/N)?: " sta
				if [[ $sta =~ ^[Yy]$ ]]
					then
				...
				read -p "Desea cambiar la rama para trabajar en este repositorio(Y/N)?: " rama
				if [[ $rama =~ ^[Yy]$ ]]
					then
				...
				read -p "Desea agregar cambios locales al Git (Y/N)?: " agregar
				if [[ $agregar =~ ^[Yy]$ ]]
					then
						git add .
						read -p "Press [Enter] key to continue..." readEnterKey
				...
				read -p "Desea agregar algun comentario al Git (Y/N)?: " cometario
				if [[ $comentario =~ ^[Yy]$ ]]
					then
